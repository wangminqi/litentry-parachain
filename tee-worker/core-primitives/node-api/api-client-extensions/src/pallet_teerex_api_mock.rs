...
use itp_types::{Enclave, IpfsHash, RuntimeConfigCollection, ShardIdentifier};
...
impl<Runtime: RuntimeConfigCollection> PalletTeerexApi<Runtime> for PalletTeerexApiMock {
	fn enclave(&self, index: u64, _at_block: Option<Runtime::Hash>) -> ApiResult<Option<Enclave>> {
		Ok(self.registered_enclaves.get(index as usize).cloned())
	}

	fn enclave_count(&self, _at_block: Option<Runtime::Hash>) -> ApiResult<u64> {
		Ok(self.registered_enclaves.len() as u64)
	}

	fn all_enclaves(&self, _at_block: Option<Runtime::Hash>) -> ApiResult<Vec<Enclave>> {
