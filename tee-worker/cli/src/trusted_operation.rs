...
fn send_request(
	...
	let xt = compose_extrinsic!(chain_api.clone(), TEEREX, "call_worker", request);
	...
	loop {
		let ret: ProcessedParentchainBlockArgs = chain_api
			.clone()
			.wait_for_event::<ProcessedParentchainBlockArgs>(&mut subscription)
			.unwrap();
